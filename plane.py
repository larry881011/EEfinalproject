...
def refraction_or_reflection(light1,interface,case):  
	...
	if case == 0:
		if n2>n1:
			re_beam	= light(wl=light1.wl, origin=light1.source, direction=reflection_direction, length=l+1/2 * light1.wl, amplitude=sqrt(R))
		else:
			re_beam	= light(wl=light1.wl, origin=light1.source, direction=reflection_direction, length=l, amplitude=sqrt(R))
...
# INTERFACE
	width = 0.000000005
	Xoil, Yoil, Zoil = 0,width,0				# a point on the air-oil interface
	...
	Xwater, Ywater, Zwater = 0,0,0			# a point on the oil-water interface
...
## MEAN_amplitude IN EACH SMALL AREA			
		...
		# the last light has index 1000 hence ignore
			if area_index < area_divide: 
				...
				light_sum[area_index,0] += sum_amplitude**2
...
## SHOW ON SCREEN
	...
	for i in range(len(all_light_sum[0])):
		...
		d = gdots(color = 64*vec(all_light_sum[2][i,0],all_light_sum[1][i,0],all_light_sum[0][i,0]))
		dots.append(d)
