...
wavelength = [480,580,680] #blue,yellow,red
...
if __name__ =="__main__":
	...
	blue = gcurve(color = color.blue)
	yellow = gcurve(color = color.yellow)
	red = gcurve(color = color.red)
	all_light_sum = []
	...
	for k in range(len(wavelength)):
		...
		for i in range(precise):
			for j in range(1):
				...
				beam = light(wavelength[k]*10**(-7),vec(-5+0.001/precise*i,5+0.0001/precise*i,0/precise*j),vec(math.cos(pi/4),-math.sin(pi/4),0))
				beams.append(beam)
	...
	for i in range(len(all_light_sum[0])):
		...
		blue.plot(pos=(0.000001*i,all_light_sum[0][i,0]))

	for i in range(len(all_light_sum[1])):

		yellow.plot(pos=(0.000001*i,all_light_sum[1][i,0]))

	for i in range(len(all_light_sum[2])):

		red.plot(pos=(0.000001*i,all_light_sum[2][i,0]))